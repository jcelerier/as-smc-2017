...
and scenario_process_TC scenario tc  =
  ...
  let execute_tc scenario tc =
    ...
    let () = Printf.printf "=====> startrf intervals length %d\n" (List.length started_intervals) in
...
and tick_scenario scenario dur pos offset =
  ...
  let rec process_root_tempConds scenario tc_list (l1, l2) =
    ...
    | h::t ->
        ...
        let (scenario, conds, funs) =
            ...
            let () = Printf.printf "=====> running intervals length %d\n" (List.length (List.filter is_interval_running scenario.intervals)) in
  ...
  let () = Printf.printf "=====> running intervals length %d\n" (List.length (running_itvs scenario)) in
...
let rec main_loop root graph duration granularity e =
  if duration > 0
  then
    let (root, funs) = tick_interval root granularity 0 in
