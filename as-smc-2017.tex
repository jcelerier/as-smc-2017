...
\begin{itemize}
\item If it is true: the previous intervals are stopped, and the next intervals are started, and it is marked as happened. 
...
\begin{tikzpicture}[scale=0.8,line cap=rect]
  ...
  \draw [line width=1.5](0, 6.4cm) -- (15, 6.4cm);
\end{tikzpicture}
\caption{A loop. The small bins represent individual audio samples ; the bigger bins represent the tick rate of the sound card. For the sake of the example, one can assume that the automation curve is used to control the output volume of the englobing scenario, not represented here.}
\label{fig.loopautom}
\end{figure}~
\begin{table}[h]
\centering
